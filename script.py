...
def scanSubDomain(domain_dir, domain):
    ...
    if not os.path.exists(subfinder_result_file): 
        ...
        subprocess.run(["subfinder", "-d", domain, "-rl", "20", "-silent", "-o", subfinder_result_file]) 
    ...
    if not os.path.exists(live_sub_list_file): 
        ...
        subprocess.run(["dnsx", "-l", "-t", "1", targets_list_file, "-silent","-o", live_sub_list_file]) # запускаем утилиту dnsx с входным файлом targets_list_file и выходным файлом live_sub_list_file
...
def scan_nmap(domain, directory, cookie):
    ...
    if not os.path.exists(os.path.join(directory, temp_xml_file)): # если файл не существует
        ...
        subprocess.run(["nmap", ip_addr, "-T4", "--max-retries", "2", "-oX", temp_xml_file]) 
    ...
    if not os.path.exists(os.path.join(directory, "nmap.xml")): 
        ...
        subprocess.run(["nmap", ip_addr, "-T4", "-sC", "-sV", "-p" + port_range, "--max-retries", "2", "-oX", nmap_xml_result_file, "-oN", nmap_plain_result_file]) 
...
def run_httpx(directory, cookie):
    ...
    if not os.path.exists(httpx_output_file):
        ...
        subprocess.run(["httpx", "-l", open_ports_file, "-t", "1", "-rl", "20", "-silent", "-H", f"Cookie: {cookie}", "-status-code", "-follow-redirects", "-tech-detect", "-fc", "500,501,502,503,504,505,403,401,404,405,400", "-server", "-websocket", "-ip", "-cname", "-asn", "-cdn", "-location", "-x", "ALL", "-tls-probe", "-o", httpx_output_file])
        
        logger.info("Starting httpx silent")
        subprocess.run(["httpx", "-l", open_ports_file, "-t", "1", "-rl", "20", "-silent", "-H", f"Cookie: {cookie}", "-o", httpx_silent_output_file])
...
def run_katana(directory, cookie):
    ...
    if not os.path.exists(katana_output_file):
        ...
        subprocess.run(["katana", "-u", httpx_silent_output_file, "-d", "10", "-jc", "-f", "url", "-silent", "-ef", "css,png,jpg,jpeg,gif,mp3,mp4,bmp,ico,svg", "-p", "1", "-ct", "5000", "-c", "1", "-rl", "20", "-kf", "all", "-H", f"Cookie: {cookie}", "-o", katana_output_file])
    ...
    #     subprocess.run(["/home/the14st/go/bin/fff", "-d", "1000", "-x", "http://localhost:8080", "-H", f"Cookie: {cookie}"], stdin=f, stdout=outfile) 
...
def run_nuclei(directory, cookie):
    ...
    nuclei_templates_path = os.path.expanduser('/home/ubuntu/nuclei-templates/http/cves/')
    ...
    if not os.path.exists(nuclei_output_file_txt) or not os.path.exists(nuclei_output_file_js):
        ...
        subprocess.run(["nuclei", "-l", open_ports_file, "-t", nuclei_templates_path, "-H", f"Cookie: {cookie}", "-stats", "-s", "info,low,medium,high,critical", "-retries", "1", "-rl", "10", "-jle", nuclei_output_file_js, "-o", nuclei_output_file_txt, "-silent"])
